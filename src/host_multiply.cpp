...
void host_multiply(const std::vector<limb_t> &A, const std::vector<limb_t> &B,
    ...
    constexpr uint32_t MODULI[NUM_MODULI] = {2013265921, 1811939329, 2113929217, 2013265921};
    ...
    for (int i = 0; i < NUM_MODULI; i++) {
        uint32_t p = MODULI[i];
        // keep a host copy to compute CPU reference NTT
        std::vector<uint32_t> A_before = A_pad;
        std::vector<uint32_t> B_before = B_pad;
